...
abstract public class MobiComConversationFragment extends Fragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        sentIcon = ContextCompat.getDrawable(getActivity(), R.drawable.applozic_ic_action_message_sent);
        deliveredIcon = ContextCompat.getDrawable(getActivity(), R.drawable.applozic_ic_action_message_delivered);
        ...
        bgShape.setColor(ContextCompat.getColor(getActivity(), applozicSetting.getSendButtonBackgroundColor()));
...
//    }
    ...
    private void disableSendMessageContainer() {
        individualMessageSendLayout.setVisibility(View.INVISIBLE);
        ConversationActivity.TAKE_CHAT_ALLOW = true;
    }

    private void checkConversationIsValid(Message message) {
        Log.d("getmesaage",message.toString());
        if (message.getContentType() == Message.ContentType.CHANNEL_CUSTOM_MESSAGE.getValue()) {
            switch (message.getMessage()) {
                case "Buddy Up Cancelled":
                    disableSendMessageContainer();
                    break;
                case "Pick Up Cancelled":
                    disableSendMessageContainer();
                    break;
    ...
    public void addMessage(final Message message) {
        this.getActivity().runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                checkConversationIsValid(message);
...
//    }
    ...
    public void sendMessage(String message, Map<String, String> messageMetaData, FileMeta fileMetas, String fileMetaKeyStrings, short messageContentType) {
        ...
        messageMetaData.put("idschedule", ((MobiComKitActivityInterface) getActivity()).getScheduleId());
...
//    }
    ...
    public void onResume() {
        ...
        if (channel != null) {
            ...
            if (!ConversationActivity.TAKE_CHAT_ALLOW) {
                individualMessageSendLayout.setVisibility(View.VISIBLE);
            } else {
                individualMessageSendLayout.setVisibility(View.INVISIBLE);
            }
    ...
    public class DownloadConversation extends AsyncTask<Void, Integer, Long> {
        ...
        protected void onPostExecute(Long result) {
            ...
            if (messageList != null && messageList.size() > 0) {
               // Log.d("getmesaage",(messageList.get(0)));
                //Toast.makeText(getActivity(), "message last item " + messageList.get(messageList.size()-1).getMessage(), Toast.LENGTH_LONG).show();
                checkConversationIsValid(messageList.get(0));//
            }
        }
    }
